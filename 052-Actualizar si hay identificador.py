...
def seleccionaArbol(event):
    ...
    try:
        ...
        for i in range(0,len(valores)):
            listacampos[i].delete(0,tk.END)
            listacampos[i].insert(0,valores[i])
    except:
    ...
    for uncampo in listacampos:
        ...
        if contador > 0:
            ...
            if operacion == "actualizar":
                peticion += tablaactual + "." + arbol['columns'][contador] + " = '" + uncampo.get() + "',"
        contador += 1
    peticion = peticion[:-1]
    if operacion == "actualizar":
        peticion += " WHERE Identificador = "+str(identificador)
    if operacion == "insertar":
        peticion += ")";
    print(peticion)
    if envias == 0:
